...
void test_segment_map()
{
    ...
    assert((new_memory->segments[0]->length) == 4);
    segment_free(new_memory);
...
void test_segment_unmap()
{
    ...
    assert(new_memory->segments[1] == NULL);
    assert(new_memory->num_unmapped == 1);
...
void test_segment_map_unmap()
{
    ...
    fprintf(stderr, "index: %d\n", segment_map(new_memory, 9));  
    fprintf(stderr, "num mapped: %d num unmapped: %d\n", new_memory->num_mapped, new_memory->num_unmapped);  
    segment_unmap(new_memory, 1);
    // 
    // /* segment_unmap(new_memory, 1000); */
    // 
    assert(new_memory->num_mapped == 3);
    assert(new_memory->num_unmapped == 1);
    // 
    // /* segment_free(new_memory); */
    // 
    segment_map(new_memory, 12);
    // assert(new_memory->num_mapped == 4);
    // assert(new_memory->num_unmapped == 0);
...
void test_segment_store()
{
    ...
    assert(new_memory->segments[1]->words[4] == 58);
    assert(new_memory->segments[0]->words[0] == 31);
    assert(new_memory->segments[2]->words[3] == 22);
